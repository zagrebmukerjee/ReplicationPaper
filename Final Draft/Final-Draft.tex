...
We also demonstrate uncertainity around our coefficient estimates using
a simulation appraoch. Specifically, for each percentile of white
manufacturing job losses, we simulate 1,000 election outcomes by drawing
from the distribution around our coefficent point estimate (again, using
the results from Model 3 in Table \ref{regResult04}). In Figure
\ref{counterfactual2}, we find that, assuming the 24th percentile or
lower of white manufacturing job losses, we find that the median
...
from the simulation exercise, we find that we would only be 95\%
confident that Democrats would have won the national election had white
...
\emph{negative} (e.g., job gains) in order for the national election
outcome to have been flipped with 95\% confidence.
...
Finally, using the same simulation approach described above, in Figure
\ref{counterfactual3} we illustrate the freuqency with which Demcorats
