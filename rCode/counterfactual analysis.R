...
# get list of states for matching on different versions of state names
...
# number of draws for simulations at each percentile
num_sims <- 1000
...
for (j in 1:99) {
  ...
  temp <- model_small %>%
   ...
   for (i in 1:num_sims) {
    # draw from coeficient distribution
    coef_draw <-
    ...
    elec_votes_tmp <- elec_votes_tmp %>%
      mutate(
        trump_votes_cf  = 
          ...
          # three states are excluded from the analysis
          # including MA, Alaska, and Hawaii.
          # just keep these states with the actual vote outcomes
          case_when(
          is.na(CF_trump_votes) ~
          as.integer(trump),
          
          # give all votes to trump if trump won & election flipped
          # note this is ignoring in subtitlities in some states
          # that do not have winner-take-all systems (e.g., Maine)
          CF_trump_votes > CF_dem_votes  &
          flipped_election == 1 ~ as.integer(votes),
          
          # give 0 votes to trump if trump won & election flipped
          CF_trump_votes < CF_dem_votes  &
          flipped_election == 1 ~ as.integer(0),
          
          
          # if election outcome not changed just used observed # of votes
          TRUE ~ as.integer(trump)
        ),
        
        # apply the same rules to clinton
...
saveRDS(combined, "counterfactual_simu_results.rds")
...
# plot frequency of winning at each percentile
summary2 <- combined %>%
